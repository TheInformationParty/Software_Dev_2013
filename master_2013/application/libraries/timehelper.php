...
class TimeHelper {
	...
	public static function get_timespan_string($older, $newer) { 
		...
		if($d < 0) { 
			$m = $m - 1; 
			$d = $d + TimeHelper::get_days_for_previous_month($m2, $Y2); 
		} 
	  if($m < 0) { 
	    $Y = $Y - 1; 
	    $m = $m + 12; 
	  } 
	  $timespan_string = TimeHelper::create_timespan_string($Y, $m, $d, $H, $i, $s); 
	...
	{ 
	  ...
	  if($Y >= 1) { 
	    ...
	    $timespan_string .= TimeHelper::pluralize($Y, 'year').' '; 
	  } 
	  if($m >= 1 || $found_first_diff) { 
	    $found_first_diff = true; 
	    $timespan_string .= TimeHelper::pluralize($m, 'month').' '; 
	  } 
	  if($d >= 1 || $found_first_diff) { 
	    $found_first_diff = true; 
	    $timespan_string .= TimeHelper::pluralize($d, 'day').' '; 
	  } 
	  if($H >= 1 || $found_first_diff) { 
	    $found_first_diff = true; 
	    $timespan_string .= TimeHelper::pluralize($H, 'hour').' '; 
	  } 
	  if($i >= 1 || $found_first_diff) { 
	    $found_first_diff = true; 
	    $timespan_string .= TimeHelper::pluralize($i, 'minute').' '; 
	  } 
	  if($found_first_diff) { 
	    $timespan_string .= 'and '; 
	  } 
	  $timespan_string .= TimeHelper::pluralize($s, 'second'); 
	  return $timespan_string; 
	} 
